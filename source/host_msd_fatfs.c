...
static void USB_HostMsdFatfsDisplayFileInfo(FILINFO *fileInfo)
{
    ...
    char my_filename[] = "APP.BIN";
    if(strcmp(my_filename, fileName) == 0)
    {
    	usb_echo("File Found!!!!!!!!!!!!!!!!!!");
    	file_found_flag = 1;
    	my_file_size = fileInfo->fsize;
    }
...
static void USB_HostMsdFatfsTest(usb_host_msd_fatfs_instance_t *msdFatfsInstance)
{
    ...
    uint32_t offset = 0;
    uint32_t readBytesCnt = 256; //Default Page size in LPC54628
    uint32_t next_page_idx = 512;//GET_PAGENUM_FROM_ADDR(APP_START_ADDR); 
    char *testString;
    ...
    {
    	int readsize;
    	fatfsCode = f_open(&file, _T("1:/APP.BIN"), FA_READ);
    	if (fatfsCode)
    	    {
    	        usb_echo("error\r\n");
    	        USB_HostMsdFatfsTestDone();
        ...
        /* 1. Binary file exists. Go ahead and Erase the App Memory. */
        erase_app(APP_START_ADDR,APP_END_ADDR);

        usb_echo("App Erase Success..\n");

        while(offset < my_file_size)
        {
            fatfsCode = f_lseek(&file, offset);
            if(readBytesCnt > (my_file_size - offset))
            {
                readBytesCnt = my_file_size - offset;
            }

            fatfsCode = f_read(&file,&printBuff,readBytesCnt,&readsize);
            ...
            page_write(next_page_idx,printBuff,readBytesCnt);
            next_page_idx++;

            if((offset+256) < my_file_size)
            {
                offset += 256;
            }
            else
            {
                offset += my_file_size - offset;
            }
            
        }
        
        
    	usb_echo("File content is:::: \r\n");

//    	for(int i=0;i<my_file_size;i++)
//    	{
//    		if(i%16 == 0)
//				usb_echo("\r\n");
//    		usb_echo("%02x ",printBuff[i]);
//    	}
    }
...
void test_flsWrite(void)
{
	uint8_t wdata[256] = {[0 ... 255] = 0xF7};
	uint16_t pagenum = 1024;
	if(erase_app(APP_START_ADDR,APP_END_ADDR) == RESP_ACK)
    {
        usb_echo(":) erase_app success \n");
    }
    else
    {
        usb_echo("erase_app failure \n");
    }
