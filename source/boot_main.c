...
int main(void)
{
    ...
    boot_jp_state = GPIO_PortRead(GPIO, BOARD_INITPINS_BOOT_JP_PORT) & (1 << BOARD_INITPINS_BOOT_JP_PIN);
    ...
    if(!boot_jp_state)
    {
    	...
    	if(!boot_jp_state) // Switch D7 = PIO1_22  Pressed = Pulled Down
    	{
    ...
    else // Switch  D7 = PIO1_22 Not Pressed = Pulled Up
    {
    	/* Jump to App */
		jump_to_app();
    }
...
static void jump_to_app(void)
 {
	if(*(uint32_t *)APP_START_ADDR != 0x00)
	{
		typedef void(*pMainApp)(void);
		 pMainApp mainApplication;

		 uint32_t mainAppAddr =  (uint32_t)(APP_START_ADDR);
